...
class CubeFace:
    ...
    def getPrintedRow(self, index):
        def tile(id): return " "+id+"|"
        out = ""
        if (index == 0):
            for i in range(3):
                out += getattr(Colors.Print, self.piecesList.array[i].color) + tile(
                    self.piecesList.array[i].id)
        elif (index == 1):
            firstPiece = getattr(
                Colors.Print, self.piecesList.array[7].color) + tile(self.piecesList.array[7].id)
            secondPiece = getattr(Colors.Print, self.color) + tile("8")
            thirdPiece = getattr(
                Colors.Print, self.piecesList.array[3].color) + tile(self.piecesList.array[3].id)
...
class RubikCube:
    ...
    # The maximum (best) score is 1 when the cube is completed.
    ...
    def rotateFront(self):
        self.__rotate(self.front, [(Rows.DOWN, self.top), (Rows.LEFT, self.right), (
            Rows.TOP, self.down), (Rows.RIGHT, self.left)], False)
        return "Rotate Front"

    def rotateBack(self):
        self.__rotate(self.back, [(Rows.TOP, self.top), (Rows.LEFT, self.left),
                      (Rows.DOWN, self.down), (Rows.RIGHT, self.right)], False)
        return "Rotate Back"

    def rotateRight(self):
        self.__rotate(self.right, [(Rows.RIGHT, self.top), (Rows.LEFT, self.back), (
            Rows.RIGHT, self.down), (Rows.RIGHT, self.front)], False)
        return "Rotate Right"

    def rotateLeft(self):
        self.__rotate(self.left, [(Rows.LEFT, self.top), (Rows.LEFT, self.front),
                      (Rows.LEFT, self.down), (Rows.RIGHT, self.back)], False)
        return "Rotate Left"

    def rotateTop(self):
        self.__rotate(self.top, [(Rows.TOP, self.front), (Rows.TOP, self.left),
                      (Rows.TOP, self.back), (Rows.TOP, self.right)], False)
        return "Rotate Top"

    def rotateDown(self):
        self.__rotate(self.down, [(Rows.DOWN, self.front), (Rows.DOWN, self.right), (
            Rows.DOWN, self.back), (Rows.DOWN, self.left)], False)
        return "Rotate Down"

    def rotateFrontReverse(self):
        self.__rotate(self.front, [(Rows.DOWN, self.top), (Rows.RIGHT, self.left), (
            Rows.TOP, self.down), (Rows.LEFT, self.right)], True)
        return "Rotate Front Reverse"

    def rotateBackReverse(self):
        self.__rotate(self.back, [(Rows.TOP, self.top), (Rows.RIGHT, self.right),
                      (Rows.DOWN, self.down), (Rows.LEFT, self.left)], True)
        return "Rotate Back Reverse"

    def rotateRightReverse(self):
        self.__rotate(self.right, [(Rows.RIGHT, self.top), (Rows.RIGHT, self.front), (
            Rows.RIGHT, self.down), (Rows.LEFT, self.back)], True)
        return "Rotate Right Reverse"

    def rotateLeftReverse(self):
        self.__rotate(self.left, [(Rows.LEFT, self.top), (Rows.RIGHT, self.back),
                      (Rows.LEFT, self.down), (Rows.LEFT, self.front)], True)
        return "Rotate Left Reverse"

    def rotateTopReverse(self):
        self.__rotate(self.top, [(Rows.TOP, self.front), (Rows.TOP, self.right),
                      (Rows.TOP, self.back), (Rows.TOP, self.left)], True)
        return "Rotate Top Reverse"

    def rotateDownReverse(self):
        self.__rotate(self.down, [(Rows.DOWN, self.front), (Rows.DOWN, self.left), (
            Rows.DOWN, self.back), (Rows.DOWN, self.right)], True)
